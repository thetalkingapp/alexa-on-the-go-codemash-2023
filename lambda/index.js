...
const { getPlace } = require('./placeUtils.js'); 
...
const LaunchRequestHandler = {
    ...
    handle(handlerInput) {
        const speakOutput = 'Welcome, you can ask where you are or ask for Help. Which would you like to try?';
    ...
    handle(handlerInput) {
      ...
      if (isGeoSupported ) {
          if (context.Geolocation && context.Geolocation.coordinate) {
              // Pull geo-location coordinates from the request
              ...
              const place = getPlace(latitude, longitude);
              speakOutput = `You are in ${place.name}.`;
          } else {
...
exports.handler = Alexa.SkillBuilders.custom()
    ...
    .addErrorHandlers(
        ErrorHandler)
    .addRequestInterceptors(TestingRequestInterceptor)
